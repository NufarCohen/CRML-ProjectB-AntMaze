...
class HumanoidEnv(MujocoEnv, utils.EzPickle):
    ...
    def get_ob(self):
        ...
        #  2 +  21 + 1 + 12 +    3   +   3   +  27
        #[0-1, 2-22, 23, 24-35, 36-38, 39-41, 42-68]
    ...
    def set_state_with_obs(self, obs_state):
        """
        NOTE: 
        This set state is deprecated since it cannot faithfully set it to a specific state
        set the env state given full obs state human Ant Maze
        """
        raise DeprecationWarning
        # data = mujoco.MjData(self.model)

        assert len(obs_state) == 69
        # len_qpos = len(self.data.qpos)
        qpos = self.data.qpos.copy()
        qvel = self.data.qvel.copy()
        xy = obs_state[:2]
        qpos[:2] = xy
        qpos[7:] = obs_state[ 2:(2+21) ]
        qvel[:] = obs_state[-27:]
        # assert len(qpos) + len(qvel) == 29
        self.set_state(qpos, qvel)


    def set_state_with_full(self, full_state):
        """
        This one can set the state by setting qpos and qvel
        """
        assert len(full_state) == 28 + 27 ## qpos + qvel
        self.set_state(full_state[0:28], full_state[28:])
