...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    def set_state_with_obs(self, obs_state):
        """
        set the env state given full obs state for Ant Maze
        """
        ## obs_state: (29,)
        assert len(obs_state) in [29, 42]
        len_qpos = len(self.data.qpos)
        qpos = obs_state[ :len_qpos ]
        qvel = obs_state[ len_qpos: ]
        assert len(qpos) + len(qvel) in [29, 42]
        self.set_state(qpos, qvel)
