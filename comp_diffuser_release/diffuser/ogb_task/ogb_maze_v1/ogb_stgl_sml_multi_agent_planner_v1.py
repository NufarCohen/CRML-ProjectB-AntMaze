...
class OgB_Stgl_Sml_MultiAgents_MazeEnvPlanner_V1:
    ...
    def __init__(self, args_train, args) -> None:
        ...
        # For now num_agents=2 by default, but all obs/action slicing below goes
        # through MultiAgentLayout so the planner is not hardcoded to two ants.
        ...
        self.carrier_order = getattr(args, "carrier_order", list(range(1, self.num_agents + 1)))

        self.handoff_x = float(getattr(args, "handoff_x", 10.0))
        self.handoff_y = float(getattr(args, "handoff_y", 10.0))
        self.handoff_points = [
            np.array([self.handoff_x, self.handoff_y], dtype=np.float32)
        ]

        self.ball_handoff_threshold = float(getattr(args, "ball_handoff_threshold", 1.0))
    ...
    def create_env(self):
        ...
        if Is_OgB_Robot_Env:
            utils.print_color(
                f"[MultiAgent Planner] loading real rollout env with gym.make: {self.multi_agent_env_name}",
                c="c",
            )

            # Importing ogbench registers the custom OGBench envs.
            # We use gym.make directly here because ogb_load_env_kwargs parses OGBench
            # dataset names and can map the new multi-agent name back to the original env.
            self.env = gym.make(
            ...
            # Gymnasium may wrap the env. MuJoCo-specific calls should use the unwrapped env.
            ...
            # Keep the original single-ant dataset name for eval problems and inverse-model path.
            # The physical rollout env is still the multi-agent env.
        ...
        utils.print_color(f'[setup_load] self.base_env.max_episode_steps={self.base_env.max_episode_steps}')
    ...
    def load_inv_model(self):
        ...
        if hasattr(self.args, 'inv_model_path'):
            ...
            ## use default one
            self.inv_model_path = utils.ogb_get_inv_model_path(self.base_env.name, gl_dim=self.dfu_ndim)
    ...
    def ma_setup_relay_coordinator_for_episode(self, final_goal_xy: np.ndarray):
        self.relay_coordinator = MultiAgentRelayCoordinator(
            MultiAgentRelayConfig(
                ...
                ball_handoff_threshold=self.ball_handoff_threshold,
        ...
        utils.print_color(
            ...
            f"final_goal_xy={final_goal_xy}, "
            f"ball_handoff_threshold={self.ball_handoff_threshold}, "
    ...
    def ma_update_relay_and_get_tasks(self, update_relay: bool = True) -> dict:
        ...
        Important:
            ...
            After that, reuse the returned active_agent_id for obs/action in
            the same timestep. Otherwise the coordinator may be updated twice.
        """
        if self.relay_coordinator is None:
            raise RuntimeError("relay_coordinator was not initialized for this episode")

        world_state = self.ma_get_world_state()
        prev_stage = self.relay_coordinator.current_stage_name

        if update_relay:
            self.relay_coordinator.update(world_state)

        new_stage = self.relay_coordinator.current_stage_name
        self.ma_stage_switched_last_update = bool(update_relay and prev_stage != new_stage)

        if self.ma_stage_switched_last_update:
            utils.print_color(
                f"[MA Relay] stage switch: {prev_stage} -> {new_stage}",
                c="y",
            )

        return self.relay_coordinator.get_tasks(world_state)


    def ma_get_current_active_agent_id(self, update_relay: bool = False):
        if self.relay_coordinator is None:
            return self.carrier_order[0]

        if update_relay:
            tasks = self.ma_update_relay_and_get_tasks()
        else:
            world_state = self.ma_get_world_state()
            tasks = self.relay_coordinator.get_tasks(world_state)

        # If the relay is done, there is no active agent anymore.
        # The rollout loop must catch this and end the episode.
        ...
        if len(active_agents) != 1:
            raise RuntimeError(
                f"Mode B expects exactly one active agent, got {active_agents}. "
                f"tasks={tasks}"
            )
    ...
    def ma_get_active_task(self, update_relay: bool = False) -> tuple:
        ...
        By default this does not update the coordinator again.
        """
        if self.relay_coordinator is None:
            return self.carrier_order[0], {
                "active": True,
    ...
    def ma_build_stage_goal_obs(self, base_goal_obs: np.ndarray, active_task: dict) -> np.ndarray:
        """
        Build an obs42-like goal for the current relay stage.

        For AntSoccer:
            - before handoff: target_xy is handoff_xy
            - after handoff: target_xy is final_goal_xy

        We preserve the original goal observation and only replace ball xy
        at dims 15:17. This keeps all non-target dimensions compatible with
        the single-agent model and normalizers.
        """
        goal_obs = np.asarray(base_goal_obs, dtype=np.float32).copy()

        target_xy = active_task.get("target_xy", None)
        if target_xy is not None and goal_obs.shape[-1] == 42:
            goal_obs[(15, 16),] = np.asarray(target_xy, dtype=np.float32)
    ...
    def ogb_env_interact_1_ep(self, pick_traj, start_state, target):
        ...
        ** OGBench **
        ...
        # n_max_steps = self.base_env.max_episode_steps
        self.n_max_steps = len(pick_traj) * self.n_act_per_waypnt + 30

        for i_et in range(self.n_max_steps):
            ## maze2d: np (4,)
            active_agent_id = self.ma_get_current_active_agent_id(update_relay=True)
            if active_agent_id is None:
                utils.print_color(
    ...
    def ogb_plan_once(self, pl_seed=None, given_probs=None): ## num_ep
        ...
        for i_ep in range(self.ep_st_idx, self.ep_st_idx+num_ep):
            ...
            self.base_env.reset() ## reset dynamics changed by last rollout
            if 'antmaze' in self.base_env.name:
                ...
                # st_state is original single-ant AntSoccer obs42.
                # The real env is multi-agent obs, so we place only the active ant + ball.
            ...
            if 'antsoccer' in self.base_env.name:
                self.base_env.set_goal(goal_xy=gl_pos[(15,16),]) ## final goal of the ball
                ...
                self.base_env.set_goal(goal_xy=gl_pos[:2]) ## gl_pos is in mj xy coordinate
            ...
            target_xy = self.base_env.cur_goal_xy ## 2D goal in mj scale
            ...
            for i_et in range(self.n_max_steps):
                ## get current obs and update relay coordinator exactly once per env timestep.
                active_agent_id = self.ma_get_current_active_agent_id(update_relay=True)
                force_ma_stage_replan = bool(getattr(self, "ma_stage_switched_last_update", False))
                ...
                ## Relay stage switch must force an immediate replan, even if
                ## the regular replanning strategy is disabled.
                ...
                if i_et == 0 or is_do_repl_et:
                    ...
                    ## The relay coordinator decides the current target.
                    ## Before handoff this is handoff_xy; after handoff this is final_goal_xy.
                    _, active_task = self.ma_get_active_task(update_relay=False)
                    stage_goal_obs = self.ma_build_stage_goal_obs(gl_pos, active_task)
                    ...
                    if is_do_repl_et and i_et > 0: ## this is doing a replanning
                        cnt_repl += 1
                    ...
                    if self.is_replan == 'at_given_t' and repl_wp_list[-1] == wp_idx: ## if is last repl traj
                        ...
                        ## At episode start the no-replan trajectory should fill the full horizon.
                        ## After a relay stage switch we intentionally replan from the current wp_idx,
                        ## so the new trajectory only needs to fill the remaining horizon.
                    ...
                    if False:
                        ...
                        tmp_obs_trajs = ogb_get_rowcol_obs_trajs_from_xy_list(self.base_env, all_plan_trajs_ep)
                        ...
                        # tmp_obs_trajs = ogb_get_rowcol_obs_trajs_from_xy(self.base_env, fused_traj_ep[None,])
    ...
    def extract_ball_trajs_ev(self, all_plan_trajs_ep, do_to_ij):
        ...
        if self.is_soccer_task: ## ball trajs
            ...
            ## a list of np 2d
            if do_to_ij:
                tmp_trajs_2 = ogb_get_rowcol_obs_trajs_from_xy_list(self.base_env, tmp_all_ball_tj_list)
            else:
    ...
    def render_agv_img(self):
        ...
        self.base_env.model.geom(vc_name).rgba = np.array([0. , 0. , 0. , 0.])
