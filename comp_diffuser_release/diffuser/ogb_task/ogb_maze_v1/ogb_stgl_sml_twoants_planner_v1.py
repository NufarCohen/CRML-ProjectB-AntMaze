...
class OgB_Stgl_Sml_TwoAnts_MazeEnvPlanner_V1:
    """
    ...
    def __init__(self, args_train, args) -> None:
        ...
        self.active_agent_id = getattr(args, "two_ant_active_agent_id", 1)
        self.two_ant_env_name = getattr(args, "two_ant_env_name", "antsoccer-twoants-arena-v0")
        
        utils.print_color(
            f"[TwoAnt Planner] active_agent_id={self.active_agent_id}, "
            f"two_ant_env_name={self.two_ant_env_name}",
            c="c",
        )
    ...
    def create_env(self):
        ...
        if Is_OgB_Robot_Env:
            utils.print_color(
                f"[TwoAnt Planner] loading real rollout env with gym.make: {self.two_ant_env_name}",
                c="c",
            )
            ...
            # dataset names and can map the new two-ant name back to the original env.
            self.env = gym.make(
                self.two_ant_env_name,
            ...
            # The physical rollout env is still the two-ant env.
            self.base_env.name = self.args.dataset
            self.env.name = self.args.dataset

            utils.print_color(
                f"[TwoAnt Planner] real env loaded. "
            ...
            if self.base_env.model.nq != 37 or self.base_env.model.nv != 34 or self.base_env.model.nu != 16:
                raise RuntimeError(
                    "TwoAnt planner did not load the two-ant env correctly. "
                    f"Got nq={self.base_env.model.nq}, "
                    f"nv={self.base_env.model.nv}, "
                    f"nu={self.base_env.model.nu}. "
                    "Expected nq=37, nv=34, nu=16."
                )
    ...
    def ogb_env_get_obs(self) -> np.ndarray:
        ...
        obs = two_ant_obs_to_single_ant_obs(
            raw_obs,
    ...
    def ogb_env_interact_1_ep(self, pick_traj, start_state, target):
        ...
        ** OGBench **
        ...
        cur_obs42 = two_ant_obs_to_single_ant_obs(
        ...
        for i_et in range(self.n_max_steps):
            ...
            obs_cur = two_ant_obs_to_single_ant_obs(
                raw_obs_cur,
                active_agent_id=self.active_agent_id,
            )
            ...
            act_pred_16 = single_ant_action_to_two_ant_action(
                act_pred,
                active_agent_id=self.active_agent_id,
            )

            raw_obs_cur, rew, terminated, truncated, info = self.base_env.step(act_pred_16)
    ...
    def ogb_plan_once(self, pl_seed=None, given_probs=None): ## num_ep
        ...
        for i_ep in range(self.ep_st_idx, self.ep_st_idx+num_ep):
            ...
            if 'antmaze' in self.base_env.name:
                ...
                # The real env is two-ant obs71, so we place only the active ant + ball.
                set_two_ant_env_from_single_ant_obs(
                    self.base_env,
                    st_state,
                    active_agent_id=self.active_agent_id,
            ...
            if 'antsoccer' in self.base_env.name:
                self.base_env.set_goal(goal_xy=gl_pos[(15,16),]) ## gl_pos of the ball
            ...
            st_state_mj = two_ant_obs_to_single_ant_obs(
                raw_st_state_mj,
                active_agent_id=self.active_agent_id,
            )
